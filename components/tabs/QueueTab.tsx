import { Users, UserPlus, Search, MoveVertical, Play, Pause, Edit3, X, Check, CheckCircle2 } from 'lucide-react'

export default function QueueTab(props: any) {
   const {
      state, admin, myProfile,
      queueSubTab, setQueueSubTab,
      ...
      handleDragStart, handleDrop, draggedPlayerId, getSkillColor,
      manualPreviews = [], autoMatches = []
   } = props;

   return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
      {/* 🌟 ปรับ UI ให้ชิดขอบพอดี */}
      <div className={`sticky top-[52px] bg-slate-100/95 dark:bg-slate-950/95 backdrop-blur-md pt-4 pb-3 z-40 transition-all duration-300 border-b border-slate-200/50 dark:border-slate-800/50 -mx-4 px-4`}>
         ...
         {/* 🌟 ช่องค้นหาจัดให้เต็มความกว้างและอยู่ใต้ Tab */}
      ...
      <div className="space-y-2 pb-10 pt-4">
        {queueSubTab === 'waiting' ? (
          ...
          : (state?.waiting || []).filter((p:any) => p.name.toLowerCase().includes(searchQueue.toLowerCase())).map((p:any, i:number) => {
            ...
            return (
              <div key={p.id} draggable={admin && !isPaused} onDragStart={(e) => handleDragStart(e, p.id)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleDrop(e, p.id)} 
                   className={`p-3.5 rounded-2xl border ${isSel ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 shadow-md ring-1 ring-blue-400/50' : isPaused ? 'opacity-60 bg-slate-100 dark:bg-slate-800' : 'bg-white dark:bg-slate-800 shadow-sm'} flex items-center justify-between transition-all ${draggedPlayerId === p.id ? 'opacity-30 scale-95' : ''}`}>
                <div className="flex items-center gap-3 w-full cursor-pointer" onClick={() => toggleSelect(p.id)}>
                  {admin && !isPaused && <div className="p-1 cursor-grab active:cursor-grabbing hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition" onClick={e=>e.stopPropagation()}><MoveVertical className="w-5 h-5 text-slate-400" /></div>}
