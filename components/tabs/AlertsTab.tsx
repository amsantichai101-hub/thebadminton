import { Bell, BellOff, CheckCircle2, Search, Smartphone } from 'lucide-react'

export default function AlertsTab(props: any) {
  const {
      notifyHistory, setNotifyHistory, notifyPerm, requestNotify,
      myProfile, getSkillName, getMySkillLevel, myWaitIndex, amIPlaying, triggerNotification
  } = props;
       ...
       <button onClick={async () => {
          try {
              const res = await fetch('/api/webpush', {
                  ...
                  body: JSON.stringify({ action: 'send', userId: myProfile?.id, title: 'Test Debug', message: 'กำลังทดสอบยิงจาก Backend' })
              });
              const data = await res.json();
              if (!res.ok) alert('❌ Backend Error: ' + JSON.stringify(data));
              else alert('✅ Backend ยิงสำเร็จ!');
