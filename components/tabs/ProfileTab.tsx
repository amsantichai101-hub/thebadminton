import { User, CheckCircle2, Play, Clock, LogOut, Activity, MapPin, Settings, BarChart2, PieChart, Megaphone, Plus, Monitor, Download, CalendarX, Trash2, RefreshCw } from 'lucide-react'

export default function ProfileTab(props: any) {
  const {
    myProfile, getSkillColor, getMySkillLevel, getSkillName,
    realPlayCount, realPlayTime, openSignOut, myPlayHistory,
    admin, auth, logout, state, refresh,
    globalPreview, setGlobalPreview, enableSound, setEnableSound,
    playStartTime, setPlayStartTime, playEndTime, setPlayEndTime, savePlayTime,
    matchMode, setMatchMode, executeAutoMatch, openBroadcastModal, openAddMember,
    openCourtManager, setFullscreen, exportRegisteredToday, showAnalyticsMenu,
    showDailyReportMenu, resetDay, clearBrowserData, APP_VERSION, openCheckIn
  } = props;
          ...
          {admin && (
            <div className="space-y-4">
               ...
               <div className="bg-slate-900/80 p-3 rounded-xl border border-slate-700 shadow-inner space-y-3">
                 <div className="flex justify-between items-center p-1 cursor-pointer" onClick={async()=>{const val = !(state?.autoMatch||false); await fetch('/api/config',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'set',key:'AutoMatch',value:val.toString()})}); refresh(false);}}>
                 ...
                 <div className="flex justify-between items-center p-1 cursor-pointer" onClick={async()=>{const val = !globalPreview; setGlobalPreview(val); await fetch('/api/config',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'set',key:'GlobalShowPreview',value:val.toString()})}); refresh(false);}}>
                 ...
                 <div className="flex justify-between items-center p-1 cursor-pointer" onClick={() => { const val = !enableSound; setEnableSound(val); localStorage.setItem('enableSound', val.toString()); }}>
               ...
               <div className="grid grid-cols-2 gap-2 mt-2">
                 ...
                 <button onClick={showDailyReportMenu} className="bg-slate-700 hover:bg-slate-600 text-white text-[11px] font-bold py-3 rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition mt-1"><BarChart2 className="w-4 h-4"/> Daily Report CSV</button>
